...
const PreDefined: React.FC = () => {
  ...
  const [expression] = useState('sinh(x)');
  const [resultSimpson, setResult] = useState('');
  const [resultTrapezoid, setResult2] = useState('');
  ...
  const simpsonCalc = (a: number, b: number, n: number, expression: string) => {
    ...
    let resultSimpson = (delta / 3) * (math.evaluate(expression, { x: a }) + math.evaluate(expression, { x: b }) + 2 * sum2 + 4 * sum1);
    setResult(resultSimpson.toString());
  };
  ...
  const trapezoidCalc = (a: number, b: number, n: number, expression: string) => {
    ...
    if (divergentIntervals.length > 0) {
      setResult2(`Error: Denominator function is divergent in the following interval(s): ${divergentIntervals.join(', ')}`);
      return;
    }
  
    let resultTrapezoid = (delta * (math.evaluate(expression.replace('x', a.toString())) + math.evaluate(expression.replace('x', b.toString()))) / 2) + (delta * sum);
    setResult2(resultTrapezoid.toString());
  };
  ...
  return (
    <div>
      ...
      <p> Simpson Result: {resultSimpson}</p>
      <p> Trapezoid Result: {resultTrapezoid}</p>
    </div>
  );
};
